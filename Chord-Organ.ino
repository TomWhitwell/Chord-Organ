...
float OLD_FREQ[SINECOUNT] = {
  55, 110, 220, 440, 880, 1760, 3520, 7040
};


float NEW_FREQ[SINECOUNT] = {
  55, 110, 220, 440, 880, 1760, 3520, 7040
};
...
float OLD_AMP[SINECOUNT] = {
  0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9
};

float NEW_AMP[SINECOUNT] = {
  0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9
};
...
<<<<<<< HEAD:Chord-Organ/Chord-Organ.ino
...
=======
    ...
    ledWrite(waveform);
    changed = true;
>>>>>>> parent of 1527bc3... working well, removed envelopes, some clicking:Chord-Organ.ino
...
>>>>>>> parent of 1527bc3... working well, removed envelopes, some clicking:Chord-Organ.ino
  ...
  // update oscillators with smoothed frequencies

<<<<<<< HEAD:Chord-Organ/Chord-Organ.ino
  if (count < N) {  // step through until we've reached number of steps

    for (int i = 0; i < SINECOUNT; i++) {
      v = count / N;
      v = SMOOTHSTEP(v);
      x = (NEW_FREQ[i] * v) + (OLD_FREQ[i] * (1 - v));
      FREQ[i] = x;
      x = (NEW_AMP[i] * v) + (OLD_AMP[i] * (1 - v));
      AMP[i] = x;
